...
function Formwork(template, properties) {
  ...
  tmp && tmp.forEach(function(i) {
    ...
    var value = typeof rawValue === 'string' ? '\'' + rawValue + '\'' : rawValue; 
    template = template.replace(i, value);
  });

  var vmContext = {require: require};

  // Evaluate the result
  vm.runInNewContext('var tmp = ' + template + '; beans = tmp.beans; types = tmp.types;', vmContext);

  // Register modules as type
  vmContext.types && vmContext.types.forEach(function(type) {
    this.registerType(type);
  }, this);

  // Build beans
  vmContext.beans && vmContext.beans.forEach(function(beanConfig) {
...
Formwork.prototype.getRealValue = function(argument) {
  ...
  if(argument.type === 'Reference') {
    // TODO find a bean config which hasn't been created when needed to create another one
    if(typeof argument.value !== 'object')
      return this.references[argument.value];
    else if(typeof argument.value === 'object' && argument.value.hasOwnProperty('reference'))
      return this.buildBean(argument.value);
    else
