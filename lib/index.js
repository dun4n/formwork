...
function Formwork(template, properties) {
  ...
  this._rawConfig = template;
}

Formwork.prototype = new EventEmitter();

Formwork.prototype.init = function() {
  // Evaluate the result
  var config = eval('(' + this._rawConfig + ')');

  // Register event handler from config
  if(config.on) {
    Object.keys(config.on).forEach(function(eventName) {
  ...
  install(this, config.dependencies, function() {

    // Register modules as type
    config.types && config.types.forEach(function(type) {
      this.registerType(type);
    }, self);

    // Build beans
    config.beans && config.beans.forEach(function(beanConfig) {
  ...
  if(argument.type === 'Reference') {
    ...
    if(typeof argument.value !== 'object') {
      ...
      return (function f(obj, paths) {
        ...
        if(!obj) {
          if(paths.length > 0)
            return f(self.references[key], paths);
          else
