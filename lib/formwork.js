// TODO create more than one Formwork instance if needed
'use strict';
...
// EventEmitter inheritance
Formwork.prototype = Object.create(EventEmitter.prototype);

Formwork.prototype.init = function(template, properties, npmOptions) {
  if(this.initialized)
    throw new Error('Formwork has already been initialized');
  ...
  install(this, this.config.dependencies, npmOptions, function(err) {
    if(err)
      throw new Error(err);
  ...
  var toRun = function() {
    var instance = (function() {
      if(!bean.hasOwnProperty('type') && !bean.hasOwnProperty('constructor')) {
        return this.getRealValue(bean.value);
      } else {
        var clazz;
        ...
        return construct(clazz, constructorArgs);
      }
    }).bind(this)();
    ...
    this.emit('bean', instance, bean.reference);
  }.bind(this);

  // bind the bean creation to a domain, usefull for exception management
  var dm = domain.create();
  dm.on('error', (function(name) {
    return function(err) {
      console.warn('[' + name + '] ' + err.stack);
    }
  })(bean.reference));
...
Formwork.prototype.register = function(reference, bean) {
  if(!this.references[reference]) {
    this.references[reference] = bean;
  }
};
