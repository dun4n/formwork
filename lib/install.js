...
module.exports = function install(fw, dependencies, done) {
  ...
  (function i(queue) {
    ...
    if(FS.existsSync(Path.join(nodeModulesPath, name))) {
      ...
      if(semver.satisfies(pkgDesc.version, version)) {
        fw.emit('dependency', name);
        return i(queue);        
      }
    }

    // Check for a local module
    var uri = FS.existsSync(Path.resolve(version)) ? version : [name, version].join('@');
    ...
    exec(['npm', 'install', uri].join(' '), execOptions, function(err) {
      if(!err) {
        ...
        throw new Error('Failed to install dependency ' + name + ' (' + dependencies[name] + ')');
      }
    });
